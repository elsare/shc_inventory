                    ...
                    <div class="portlet light">
                        <div class="portlet-title">
                            <div class="caption font-dark">
                                <span class="caption-subject bold uppercase">Part Number</span>
                            </div>
                        </div>
                        <div class="dataTables_wrapper">
                            <div class="col-md-12 col-sm-12">
                                        {{view('partials.buttons.datatable',
                                            ['create' => ['route' => route('Master.PartNumber.create'),
                                                          'name' => 'Part No.']
                                            ])
                                        }}
                             </div>
...
<script>
    $(document).ready(function(){
        var table = $('#datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('Master.PartNumber.dataTable') }}",
            columns: [
    ...
    const deleteData = async (id) => {
           const res = await fetch(`{{ route('Master.PartNumber.destroy','') }}/${id}`,{

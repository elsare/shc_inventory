                        ...
                        <div class="dataTables_wrapper">
                            <div class="col-md-12 col-sm-12">
                                    <!-- {{view('partials.buttons.datatable',
                                            ['create' => ['route' => route('Transaksi.Gap.create'),
                                                          'name' => 'Actual']
                                            ])
                                        }} -->   
                             </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="model" action="{{ route('Transaksi.Gap.store') }}" method="POST" enctype="multipart" enctype="multipart/form-data">
                    @csrf
...
<script>
    $(document).ready(function(){
        var table = $('#datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('Transaksi.Gap.dataTable') }}",
            columns: [
    ...
    const deleteData = async (id) => {
           const res = await fetch(`{{ route('Transaksi.Gap.destroy','') }}/${id}`,{
    ...
    function myFunction(id) {
      window.open(`{{ route('Transaksi.Gap.show','') }}/${id}`);
    }
</script>
@endsection